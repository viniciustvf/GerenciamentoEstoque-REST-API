...
public class MovementServiceImpl implements MovementService {
	...
	public List<Movement> findByEntry(Entry entry) {
		...
		if (lista.isEmpty()) {
			throw new ObjectNotFound(
					"Nenhum movimento de entrada encontrado para a entrada %s".formatted(entry.getId()));
		}
		return lista;
	}
	...
	@Override
	public List<Movement> findByDateTimeContainsDate(ZonedDateTime date) {
		List<Movement> lista = repository.findByDateTimeBetween(DateUtils.zonedDateTime0000(date),
				DateUtils.zonedDateTime2359(date));
		if (lista.isEmpty()) {
			throw new ObjectNotFound(
					"Nenhum movimento encontrado para a data %s".formatted(DateUtils.zonedDateTime0000(date)));
		}
		return lista;
	}
